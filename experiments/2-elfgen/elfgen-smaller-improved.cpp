...
int main(){
    ...
    elf << (uint32_t)(1+4);
    //the location of elf header padding
    elf << (uint64_t)0x8;//where in the file is the code
    
    elf << (uint64_t)0x40008;//where put it in memory

    //physical address (for systems where it's relevant???)
    elf << MOVZ_imm16(true,93,r8);
    elf << SVC();
    
    elf << (uint64_t)110;//size in file
    elf << (uint64_t)110;//size in memory
    elf << (uint64_t)0x1000;//allignment


    elf.dumpfile("./realmin-improved");
}